...
// 1. Call GET https://dogapi.dog/api/v2/breeds and read results from the JSON:API response under data[*].attributes.
// 2. Pagination - The API may return paginated results.
// 3. If the response contains links.next, keep fetching that URL and append data items.
// 4. Stop when links.next is missing or null.
//
// Fields to use
// 1. From attributes, use:
//    1.1 name
//    1.2 life
//    1.3 female_weight
// 2. Each of life and female_weight may be:
//    2.1 an object: { "min": <number>, "max": <number> }, or
//    2.2 a range string: e.g. "10 - 12", "10–12", "10 to 12", "20 - 25 kg".
//
// Normalization rules:
...
// - Handle separators: -, –, —, and "to".
// - Strip unit suffixes like kg, kgs and whitespace before parsing.
// - If a value is missing or can't be parsed, exclude that breed from that specific ranking.
//
// Rankings to output (exactly 10 items when possible):
// - Top 10 by max life span (descending)
// - Top 10 by max female weight (descending)
// - Tie-breaker: sort by name ascending.
//
// Error handling:
// - Handle HTTP failures / timeouts / invalid JSON gracefully.
// - If pagination is missing, stop fetching.

class Program
{
    static async Task<int> Main(string[] args)
    {
        // *** STEP 1: Accumulate all breed data across
        // all pages
        var breeds = new List<BreedInfo>();

        // *** STEP 2: HTTP Fetching with Pagination
        // with Catch Block
        try
        {
        ...
        // STEP 6a: Print Top 10 by Max Life Span
        Console.WriteLine("Top 10 by Max Life Span");
        Console.WriteLine();

        int lifespanCount = Math.Min(10, topLifespan.Count);
        for (int i = 0; i < lifespanCount; i++)
        {
            var b = topLifespan[i];
            Console.WriteLine($"{i + 1}. {b.Name} — max_lifespan={b.MaxLife!.Value}");
            Console.WriteLine();
        }

        // STEP 6b: Print Top 10 Heaviest Female Breeds
        Console.WriteLine("Top 10 Heaviest Female Breeds");
        Console.WriteLine();

        int weightCount = Math.Min(10, topWeight.Count);
        for (int i = 0; i < weightCount; i++)
        {
            var b = topWeight[i];
            Console.WriteLine($"{i + 1}. {b.Name} — max_female_weight={b.MaxFemaleWeight!.Value}");
            Console.WriteLine();
        }
    ...
    // STEP 1: BreedInfo Record (Transformed)
